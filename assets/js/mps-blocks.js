                ...
                var unsub = onPaymentSetup(function(){
                    ...
                    if(blockedHit){
                        return {
                            ...
                            message: blockedHit.message || (dataVar.blocked_message || 'This card cannot be used at this store. Please try a different card.')
                        };
                    }
            ...
            cardFields.forEach(function(f, i){
                ...
                if(f.name === 'card_number'){
                    ...
                    onFieldChange = function(e){
                        ...
                        setBinBlocked(hit
                            ? (hit.message || dataVar.blocked_message || 'This card cannot be used at this store. Please try a different card.')
                ...
                var inputProps = {
                    type: f.type || 'text',
                    // Lets clearCardFields() find the node after a completed charge. The inputs are
                    // uncontrolled, so without a handle on the element only the React state could be
                    // cleared and the old card number would stay visible on screen.
